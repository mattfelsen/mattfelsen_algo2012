...
void particle::draw(){
	...
	//c = ofColor::fromHsb(hue, powf(displacement, 2), 50);
	//ofSetColor(c);
	ofSetColor(0);
	...
	float w = abs(prevPos.x+5 - pos.x);
	
//	float vx = w;
//	float vy = abs(prevPos.y - pos.y);
//	
//	float angle = ofRadToDeg(atan2(vy, vx));
//	
//	ofPushMatrix();
...
//	ofRotateZ(-angle);
	...
	//ofRectRounded(pos.x, pos.y, w, 2, 1);
