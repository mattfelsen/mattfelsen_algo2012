...
void creature::setup(ofColor color, float size) {
	...
	p9->setInitialCondition(pos.x, pos.y + size/2,0,0);
	particles.push_back(p9);
	
	p10->setInitialCondition(pos.x + size, pos.y + size/2,0,0);
	particles.push_back(p10);

	p11->setInitialCondition(pos.x - size/3, pos.y + size/2,0,0);
	particles.push_back(p11);
	
	p12->setInitialCondition(pos.x + size + size/3, pos.y + size/2,0,0);
